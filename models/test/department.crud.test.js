...
describe('Department', () => {
  ...
  describe('Updating data', () => {
    ...
    it('should properly update multiple documents with "updateMany" method', async () => {
      await Department.updateMany({}, { $set: { name: 'Updated Name' } });
      const department = await Department.find();
      expect(department[0].name).to.be.equal('Updated Name');
      expect(department[1].name).to.be.equal('Updated Name');
    });
  });
  ...
  describe('Removing data', () => {
    ...
    it('should properly remove one document with "deleteOne" method', async () => {
      ...
      const deletedDocument = await Department.findOne({ name: 'Department #1' });
